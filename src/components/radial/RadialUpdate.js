...
class RadialUpdate extends Component {
  ...
  drawRadial(){
    ...
    function buildClassificationTree(root){
      root.hit = 0;
      if(assignmentsArray[0].fields.classifications.includes(root.title)){
        root.children[i].hits += 1;
      }
      if(!root.children.length){
        return;
      }else{
        for(let i = 0; i < root.children.length; i++){
          buildClassificationTree(root.children[i])
        }
      }

    }

   buildClassificationTree(ontology_data)
   console.log(ontology_data)
    // straight links
