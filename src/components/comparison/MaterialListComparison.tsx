...
interface ListProps extends RouteComponentProps<MatchParams> {
    ...
    user_data: any;
    from: string;
    selectedListCallback(event: boolean, list: any): void; // call back values from child component to update comparison ids
    idlist: number[];
...
export const MaterialListComparison: FunctionComponent<ListProps> = ({   history,
                                                               location,
                                                               match,
                                                               api_url,
                                                               user_materials,
                        ...
                        input={
                            <Checkbox id={`checkbox-${value.id}`}
                                      ...
                                      onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                                          ...
                                          selectedListCallback(event.target.checked, {name:value.title, id: value.id})
                                      }}
    ...
    // let analyze = <Analyze idlist={listInfo.selected_materials} listTwo={listTwo} user_id={user_id} user_data={user_data}
                             // currentLoc="compare" from="idlist"/>;
    // var analyze = <Analyze info={listInfo.selected_materials} user_id={user_id} user_data={user_data} currentLoc="compare" from="idlist"/>
    // onGetList(listInfo.selected_materials, "idlist")
    return (
        <div>
                        ...
                        <Button className={classes.margin} variant="contained" color="primary"
                                onClick={() => {
                                    if (listInfo.materials !== null){
                                        ...
                                        selectedListCallback(true, listInfo.materials.map(function(a) {return {name: a.title, id: a.id};}))
                                    }
