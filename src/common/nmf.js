// Source: https://github.com/aneesha/nmf.js
// Javascript implementation of Non-negative Matrix Factorisation Algorithms
import numeric from 'numeric';

export var nmf = typeof exports === 'undefined' ? function nmf() {} : exports;
...
nmf.mu = function mu(A, k, maxiterations, tolerance) {
  ...
  for (i = 0; i < maxiterations; i++) {
    ...
    H = numeric.div(
      numeric.mul(H, numeric.dot(numeric.transpose(W), A)),
      numeric.add(
        numeric.dot(numeric.dot(numeric.transpose(W), W), H),
        nmf.epsilon
      )
    );
    //W = W .* (AH' ) ./ (WHH' + epsilon);
    W = numeric.div(
      numeric.mul(W, numeric.dot(A, numeric.transpose(H))),
      numeric.add(
        numeric.dot(numeric.dot(W, H), numeric.transpose(H)),
        nmf.epsilon
      )
    );
...
nmf.calculate_reconstructionError = function calculate_reconstructionError(
  A,
  A_reconstructed
) {
