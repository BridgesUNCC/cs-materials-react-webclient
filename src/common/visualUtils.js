...
function styleTreeNode(node, hitThresh) {
  ...
  if (node.assignments.length > 0)
    node.label = node.label.concat(
      node.assignments[node.assignments.length - 1].fields.title,
      ' \n'
    );
  ...
  if (tree.children) {
    ...
    for (let i = 0; i < spliced_children.length; i++) {
      tree.children.splice(
        tree.children.filter((x) => x.id === spliced_children[i].id),
        1
      );
    }
    if (tree.children) {
...
export function buildClassificationTree(
  ...
  if (ontologyType == 'acm') {
    for (let j = 0; j < assignmentsArray.length; j++) {
      if (
        assignmentsArray[j].fields.classifications.includes(
          'ACM-CS2013: ' + root.title
        )
      ) {
